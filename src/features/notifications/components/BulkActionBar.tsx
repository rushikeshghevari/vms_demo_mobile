...
export function BulkActionBar({ count, onMarkRead, onArchive, onDelete, onCancel }: Props) {
  return (
    <View className="mx-4 mb-2 flex-row items-center justify-between rounded-2xl border border-primary-200 bg-primary-50 px-3 py-2 dark:border-primary-800 dark:bg-primary-950/40">
      ...
      <View className="flex-row items-center">
        <ActionButton icon="checkmark-done-outline" label="Read" color="#16A34A" onPress={onMarkRead} />
        <ActionButton icon="archive-outline" label="Archive" color="#D97706" onPress={onArchive} />
        <ActionButton icon="trash-outline" label="Delete" color="#DC2626" onPress={onDelete} />
      </View>
    </View>
  );
}
